...
import { TopBar } from './TopBar';
import { MainHeader } from './MainHeader';
import { ItemList } from '../ui/ItemList'; // ✅ Import ItemList
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <Sidebar
        isOpen={isSidebarOpen}
        categories={categories?.map(group => group.name) || []}
        onSelectCategory={setSelectedCategory} // ✅ Handle category selection
      />
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 bg-gray-50">
          <div className="mx-auto py-6 sm:px-6 lg:px-8">
            <ItemList category={selectedCategory} /> {/* ✅ Filter products by category */}
          </div>
        </main>
      </div>
    </div>
