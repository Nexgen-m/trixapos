...
//     queryKey: ["products"],
//     queryFn: async ({ pageParam = 1 }) => fetchProducts(pageParam), // Handles pagination
...
export function useProducts(category: string) {
  return useInfiniteQuery<ProductResponse, Error>({
    queryKey: ["products", category], // Include category in query key
    queryFn: async ({ pageParam = 1 }) => fetchProducts(pageParam, category), // Pass category to API
