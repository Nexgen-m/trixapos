...
import { useInfiniteQuery } from "@tanstack/react-query";
import { useFetchProducts } from "../../hooks/fetchers/posApi"; // ✅ Use the fixed fetch function
import { Item } from "../../types/pos";
...
export function useProducts(category: string) {
  return useInfiniteQuery<ProductResponse, Error>({
    queryKey: ["products", category], // ✅ Include category in query key for refetching
    queryFn: async () => {
      const { products } = useFetchProducts(category); // ✅ Use the fixed fetch function
